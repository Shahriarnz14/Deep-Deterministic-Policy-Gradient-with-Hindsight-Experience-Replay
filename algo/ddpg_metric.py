    ...
    def train(self, num_episodes, hindsight=False):
        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                y_vec = rewards + (1-done_vec) * GAMMA * self.critic.target_model.predict([new_states,
                                                                                           self.actor.target_model.predict(new_states)]).squeeze()
                ...
                value_metric_curr += self.critic.model.predict([store_states[0][None],
                                                                self.actor.model.predict(store_states[0][None])])[0]
            ...
            # Logging
            print("Episode %d: Total reward = %d\t|\tTD loss = %.2f\t,\tV-Metric = %2f\t,\tSteps = %d\t|\t Info = %s\t|\tSuccess = %s" %
                  (i, total_reward, loss / step, value_metric_curr / step, step, info['done'], success))
            if i % 100 == 0:
                ...
                self.test_TD_error_mean.append(loss/step)
                self.value_metric.append(value_metric_curr / step)
                ...
                plt.figure()
                plt.plot(x_plotting, self.test_TD_error_mean)
                plt.xlabel('Number of Training Episodes')
                ...
                plt.figure()
                plt.plot(x_plotting, self.value_metric)
                plt.xlabel('Number of Training Episodes')
                plt.ylabel('Average Value Metric')
                plt.savefig('Value_Metric.png')
                plt.close('all')
                ...
                np.save('data_TDerrorMean_Train', np.asarray(self.test_TD_error_mean))
                np.save('data_ValueMetric_Train', np.asarray(self.value_metric))
