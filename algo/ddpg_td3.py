...
class DDPG(object):
    ...
    def __init__(self, env, outfile_name):
        ...
        self.critic_1 = CriticNetwork(sess=self.sess, state_size=state_dim, action_size=action_dim,
                                      batch_size=BATCH_SIZE, tau=TAU, learning_rate=LEARNING_RATE_CRITIC)
        self.critic_2 = CriticNetwork(sess=self.sess, state_size=state_dim, action_size=action_dim,
                                      batch_size=BATCH_SIZE, tau=TAU, learning_rate=LEARNING_RATE_CRITIC)
    ...
    def train(self, num_episodes, hindsight=False):
        ...
        if hindsight:
            test_reward_figure_filename = 'TD3_Reward-HER.png'
        else:
            test_reward_figure_filename = 'TD3_Reward-DDPG.png'
        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                y_vec = rewards + (1-done_vec) * GAMMA * np.minimum(self.critic_1.target_model.predict([new_states, self.actor.target_model.predict(new_states)]).squeeze(),
                                                                    self.critic_2.target_model.predict([new_states, self.actor.target_model.predict(new_states)]).squeeze())

                loss_1 += self.critic_1.model.train_on_batch([states, actions], y_vec)
                loss_2 += self.critic_2.model.train_on_batch([states, actions], y_vec)

                if step % self.d_update_period == 0:
                    actions_optimize = self.actor.model.predict(states)
                    gradients_action = self.critic_1.gradients(states=states, actions=actions_optimize)
                    self.actor.train(states=states, action_grads=gradients_action)

                    self.critic_1.update_target()
                    self.critic_2.update_target()
            ...
            print("Episode %d: Total reward = %d\t,\tTD loss = %.2f\t,\tSteps = %d; Info = %s\t,\tSuccess = %s" %
                  (i, total_reward, loss_1 / step, step, info['done'], success))
            if i % 100 == 0:
                ...
                self.test_TD_error.append([loss_1, step]) # From Last Training
                ...
                plt.close('all')
                np.save('TD3_data_TestRewardMean', y_plotting)
                np.save('TD3_data_TestRewardStd', y_error)
                np.save('TD3_data_TestSuccessRatio', np.asarray(self.test_success_ratio))
                np.save('TD3_data_TestTDError_Train', np.asarray(self.test_TD_error))
